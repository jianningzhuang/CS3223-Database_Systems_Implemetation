...
/*
 * cs3223 Doubly Linked List Node
 */
...
void StrategyUpdateAccessedBuffer(int buf_id, bool delete)
{
	DLLNode *current_node = &StrategyControl->LRUStack[buf_id];
	...
	if (delete) /* case 4 */
	{
		...
		else if (StrategyControl->LRUStackHead == buf_id) /* remove Head */
		{
			...
			DLLNode *next_node = &StrategyControl->LRUStack[current_node->next];
		...
		else if (StrategyControl->LRUStackTail == buf_id) /* remove Tail */
		{
			...
			DLLNode *prev_node = &StrategyControl->LRUStack[current_node->prev];
	...
	else
	{
		if (current_node->next == NOT_IN_STACK && current_node->prev == NOT_IN_STACK) /* case 2 */
		{
			...
			{
				DLLNode *old_head_node = &StrategyControl->LRUStack[StrategyControl->LRUStackHead];
				StrategyControl->LRUStackHead = current_node->buf_id;
				current_node->prev = END_OF_STACK;
		...
		else /* case 1 & 3*/
		{
			...
			{
				DLLNode *new_tail_node = &StrategyControl->LRUStack[current_node->prev];
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state)
{
	...
	DLLNode *replacement_node = &StrategyControl->LRUStack[StrategyControl->LRUStackTail];
	for (; replacement_node->prev != END_OF_STACK; replacement_node = &StrategyControl->LRUStack[replacement_node->prev])
	{
		buf = GetBufferDescriptor(replacement_node->buf_id);
...
void StrategyInitialize(bool init)
{
	...
	StrategyControl = (BufferStrategyControl *)
		ShmemInitStruct("Buffer Strategy Status",
						offsetof(BufferStrategyControl, LRUStack) + NBuffers * sizeof(DLLNode),
						&found);
	...
	if (!found)
	{
		...
		/*
		 *cs3223 LRUStackHead and LRUStackTail initialised to END_OF_STACK representing empty LRUStack
		 */
		StrategyControl->LRUStackHead = END_OF_STACK;
		StrategyControl->LRUStackTail = END_OF_STACK;
		...
		{
			DLLNode *node = &StrategyControl->LRUStack[i];
			node->buf_id = i;
