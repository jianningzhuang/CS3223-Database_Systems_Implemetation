...
void StrategyUpdateAccessedBuffer(int buf_id, bool delete)
{
	...
	else
	{
		if (current_node->next == NOT_IN_STACK && current_node->prev == NOT_IN_STACK) /* case 2 */
		{
			...
			{
				DLLNode *old_head_node = &LRUStack[LRUStackControl->LRUStackHead];
				LRUStackControl->LRUStackHead = current_node->buf_id;
				current_node->prev = END_OF_STACK;
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state)
{
	...
	DLLNode *replacement_node = &LRUStack[LRUStackControl->LRUStackTail];
	while (true)
	{
...
Size StrategyShmemSize(void)
{
	...
	size = add_size(size, MAXALIGN(sizeof(LRUStackStrategyControl)));
...
void StrategyInitialize(bool init)
{
	...
	bool found_LRUStackControl;

	LRUStackControl = (LRUStackStrategyControl *)ShmemInitStruct("LRUStackControl", sizeof(LRUStackStrategyControl), &found_LRUStackControl);

	if (!found_LRUStackControl)
	{
		Assert(init);
	...
	LRUStack = (DLLNode *)ShmemInitStruct("LRUStack", NBuffers * sizeof(DLLNode), &found_LRUStack);
